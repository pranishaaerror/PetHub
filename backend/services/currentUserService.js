...
export const resolveUserRole = (firebaseUser, fallbackEmail) => {
  ...
  if (firebaseUser.customClaims?.role === "veterinarian") {
    return "veterinarian";
  }

  if (firebaseUser.customClaims?.role === "groomer") {
    return "groomer";
  }
...
export const syncFirebaseUser = async ({ uid, email }) => {
  ...
  if (user) {
    const providerRoles = new Set(["veterinarian", "groomer"]);
    const preservedRole =
      providerRoles.has(user.role) && userPayload.role !== "admin" ? user.role : null;
    user.set(userPayload);
    if (preservedRole) {
      user.role = preservedRole;
    }
    return user.save();
  }
