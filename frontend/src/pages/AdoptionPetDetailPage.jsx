...
export const AdoptionPetDetailPage = () => {
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      {/* ── Temperament + Request status ──────────────────────────── */}
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">

        {/* Temperament card */}
        <section className="rounded-[24px] bg-white p-6 shadow-[0_4px_20px_rgba(45,45,45,0.07)] md:p-8">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-[16px] bg-[#FFF5E0] text-[#F5A623]">
              <Sparkles className="h-5 w-5" />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-[#B78331]">Temperament</p>
              <h2 className="text-xl font-bold text-[#2D2D2D]">A closer feel for {pet.petName}</h2>
            </div>
          </div>

          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl bg-[#FFF8EE] p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#B78331]">Personality</p>
              <p className="mt-2 text-base font-bold text-[#2D2D2D]">{pet.temperament}</p>
              <p className="mt-2 text-sm leading-6 text-[#7A6A50]">{pet.summary}</p>
            </div>
            <div className="rounded-2xl bg-[#FFF8EE] p-5">
              <p className="text-xs font-semibold uppercase tracking-wider text-[#B78331]">Care note</p>
              <p className="mt-2 text-base font-bold text-[#2D2D2D]">Best with a gentle transition</p>
              <p className="mt-2 text-sm leading-6 text-[#7A6A50]">{pet.careNote}</p>
            </div>
          </div>
        </section>
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Profile snapshot */}
          <div className="rounded-[24px] bg-[linear-gradient(145deg,#FFF8EE,#FFFFFF)] p-5 shadow-[0_4px_20px_rgba(45,45,45,0.07)]">
            <div className="mb-4 flex items-center gap-2">
              <Info className="h-4 w-4 text-[#F5A623]" />
              <p className="text-xs font-semibold uppercase tracking-widest text-[#B78331]">Profile Snapshot</p>
            </div>
            <div className="space-y-3">
              <InfoRow icon={ShieldCheck} label="Adoption status" value={pet.displayStatus} />
              <InfoRow
                icon={CalendarDays}
                label="Intake date"
                value={new Date(pet.intakeDate).toLocaleDateString()}
              />
              <InfoRow icon={PawPrint} label="Type" value={pet.species} />
            </div>
          </div>

          {/* Request status / CTA card */}
          <div
            id="request-status"
            className="flex flex-1 flex-col gap-4 rounded-[24px] bg-[linear-gradient(140deg,#2D2D2D,#4A3828)] p-6 text-white shadow-[0_4px_20px_rgba(45,45,45,0.20)]"
          >
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-white/75">
                <HeartHandshake className="h-3.5 w-3.5" />
                Ready to connect
              </div>
              <h2 className="mt-3 text-2xl font-bold leading-snug">
                {currentMeta?.label ?? "A softer start for adoption."}
              </h2>
              <p className="mt-2 text-sm leading-6 text-white/65">
                {currentMeta?.description ??
                  "Open the request only when the home, routine, and energy match feel right. PetHub keeps things warm — never pushy."}
              </p>
            </div>

            {/* Status badge if exists */}
            {currentMeta && (
              <div className={`flex items-center gap-2 rounded-2xl border px-4 py-3 ${currentMeta.bg} ${currentMeta.border}`}>
                <currentMeta.icon className={`h-5 w-5 ${currentMeta.iconColor}`} />
                <span className="text-sm font-semibold text-[#2D2D2D]">{currentMeta.label}</span>
              </div>
            )}
            ...
            {canRequest ? (
              <Link
                ...
                className="mt-auto inline-flex items-center gap-2 self-start rounded-full bg-[#F5A623] px-5 py-2.5 text-sm font-semibold text-white shadow-[0_4px_14px_rgba(245,166,35,0.35)] hover:bg-[#e09515] transition-colors"
              >
                <HeartHandshake className="h-4 w-4" />
                Continue to request form
              </Link>
            ) : (
              <div className="mt-auto inline-flex items-center gap-2 self-start rounded-full bg-white/10 px-4 py-2 text-sm font-semibold text-white/70">
                {pet.status !== "Available" ? "Requests paused for this profile" : currentMeta?.label}
              </div>
            )}
          </div>
        </div>
